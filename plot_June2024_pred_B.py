...
df['t0'] = [ts_list[ts_list<df.ts0[ind]].max() for ind in df.index] #should be max
t1 = np.zeros(len(df.index))
for ind in df.index:
    if np.any(ts_list>df.ts0[ind]):
        t1=ts_list[ts_list>df.ts0[ind]].min()
    else:
        t1 = np.nan
df['t1'] = t1
...
for t in range(len(ts_list)):
    ...
    p=axll.pcolormesh(x_center,y_center,pmap,shading='nearest',cmap=cmo.cm.matter,norm=matplotlib.colors.LogNorm(vmin=10**-3,vmax=10**3))

    for ind in df.index:
        axll.plot(df.xsloc[ind],df.ysloc[ind],marker=Ds[df.filtered_with[ind]]['marker'],markersize=Ds[df.filtered_with[ind]]['markersize'],markerfacecolor='None',mec='gray',lw=1,zorder=50)
        if ts_list[t] in [df.t0[ind],df.t1[ind]]:
            axll.plot(df.xsloc[ind],df.ysloc[ind],marker=Ds[df.filtered_with[ind]]['marker'],markersize=Ds[df.filtered_with[ind]]['markersize'],markerfacecolor='k',mec='none',lw=2,zorder=51)
    # plt.colorbar(p)
    ...
    axll.text(0.95,0.1,dt_object.strftime("%m/%d/%Y,\n%H:%M:%S PDT"),transform=axll.transAxes,ha='right',zorder=55,fontsize=8,color='k',fontweight='bold',bbox=dict(facecolor='white'))
    axll.text(0.1,0.9,'Particle m-3 in upper 10m',transform=axll.transAxes,ha='left',fontsize=12)
    ...
    for sampling_location in df[df.filtered_with=='Ascension'].sampling_location.unique():
        ...
        mx0 = np.argwhere(maskr[yci,:])[0][0]
        mx1 = np.argwhere(maskr[yci,:])[-1][0]

        axll.plot(x_center[mx0:mx1],[y_center[yci]]*len(x_center[mx0:mx1]),linestyle='dashed',color='k',marker='none',lw=2)

        pmap_z = particle_map[t,:,yci,mx0:mx1]/dv[t,:,yci,mx0:mx1]
        ax.pcolormesh(x_center[mx0:mx1],z_center[t,:,yci,mx0:mx1],pmap_z,shading='nearest',cmap=cmo.cm.matter,norm=matplotlib.colors.LogNorm(vmin=10**-3,vmax=10**3))
        ...
        for ind in gg.index:
            ax.plot(gg.xsloc[ind],-gg.depth_m[ind],linestyle='None',marker=Ds['Ascension']['marker'],markersize=Ds['Ascension']['markersize'],markerfacecolor='None',mec='gray',zorder=500,lw=1)
            if ts_list[t] in [df.t0[ind],df.t1[ind]]:
                ax.plot(gg.xsloc[ind],-gg.depth_m[ind],linestyle='None',marker=Ds['Ascension']['marker'],markersize=Ds['Ascension']['markersize'],markerfacecolor='k',mec='none',zorder=501,lw=2)
    ...
    plt.savefig(home+f'plots/June 2024 samples/zw_time_{t}_upper10m.png')
