...
f_list.sort()
f_list = [x for x in f_list if (x[:12]=='hc_dolph2_3d')&(x[18:28]=='2023.10.16')] # look for Oct 2023 releases
...
#VL pen location 47.740037
#set dt list based on samples
Pacific = pytz.timezone("PST8PDT")
utc = pytz.utc

first_sample_utc = Pacific.localize(datetime(2023,10,16,10,0)).astimezone(utc)
last_sample_utc = Pacific.localize(datetime(2023,10,16,18,0)).astimezone(utc)
...
for f in f_list:
    ...
    for t in range(nt):
        ...
        for z in range(nz):
            z0 = z_edges[z]
            z1 = z_edges[z+1]
            zmask = (ds['z'][pt,:]>z0)&(ds['z'][pt,:]<z1)&pmask #should combine masks
...
outfn = home+'LO_data/eDNA/Oct2023_3dhist_w_ages'
if VL_only:
