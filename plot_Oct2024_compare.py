...
for t in range(len(dt_list)):
    ...
    fig = plt.figure(figsize=(fw*4,fh*1))
    dye_0lag['ax'] = fig.add_subplot(1,3,1)
    dye_1lag['ax'] = fig.add_subplot(1,3,2)
    particles['ax'] = fig.add_subplot(1,3,3)
    ...
    for mod in model_list:
        ...
        for station in particles['station_list']:
            mod['ax'].plot(station['x'],station['y'],marker='o',linestyle='none',mec='k',mfc=station['col'],markersize=8,zorder=70)
    ...
    plt.savefig(home+f'plots/Oct 2024 samples/compare/upper10m_{t:0>2}.png')
