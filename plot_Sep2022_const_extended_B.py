...
gq = dfq[dfq['campaign']=='Sept_2022']
mll2ref = {
    'Inside dolphin pen - 3 dolphins':'inside pen with 3 dolphins',
    'outside dolphin pen':'outside pen with 3 dolphins',
    'marginal pier':'marginal pier',
    'near sea lions':'near sea lions',
    'Inside dolphin pen - 1 dolphin':'inside pen with 1 dolphin'}
...
sample_dt = pdt.localize(datetime(2022,9,30,14,0)).astimezone(tz)
...
moor_fn = home+'data/Sep2022_DNA_moorings_extended_const_only_noscale.p'

moor_dict = pickle.load(open(moor_fn,'rb'))
moor_list = [moor for moor in moor_dict['moor_list'] if moor['label'] in mll2ref.keys()]
dt_list = moor_dict['dt_list']
# nmoor = len(moor_list)
nmoor = len(mll2ref.keys())
rainbow = plt.get_cmap('rainbow',nmoor)
moor_lat_list = [moor['lat'] for moor in moor_list]
# ns_inds = np.argsort(moor_lat_list)
