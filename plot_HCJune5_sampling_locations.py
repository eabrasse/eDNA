...
sampling_data_fn = 'LO_data/eDNA/June5_sampling_locations.csv'
df = pd.read_csv(sampling_data_fn,sep=',',engine='python')
df['approx_dt']=Pacific.localize(pd.to_datetime(df.date_collected+' '+df.approx_time_collected,utc=False))
# df['mid_dt']=Pacific.localize(pd.to_datetime(df.date_collected+' '+df.mid_time_collected,utc=False))

#import tidal height
mllw_data_fn = 'LO_data/eDNA/9445133_MLLW_20230603-20230605.txt'
...
for dt in dt_list0:
    ...
    with Image.open(figname) as img:
        ...
        if dt>=first_sample_utc:
            ax.scatter(df[df.approx_dt<dt]['lon'],df[df.approx_dt<dt]['lat'],marker='*',s=10,edgecolors='k',c=Paired(6),zorder=200)
            if dt in df.approx_dt:
                ax.plot(df[df.approx_dt==dt]['lon'],df[df.approx_dt==dt]['lat'],marker='*',s=10,edgecolor='k',color=Paired(7),zorder=201)
                
        ax.text(0.9,0.1,dt.astimezone(Pacific).strftime("%m/%d/%Y, %H:%M:%S PDT"),transform=ax.transAxes,ha='right',va='center',color='white')
        outfn = home+'etools/plots/June5_sampling_locations/figure_{}.png'.format(str(figcount).zfill(3))
        plt.savefig(outfn)
