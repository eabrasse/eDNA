...
dft['datetime_pdt']=[dt.astimezone(utc) for dt in dft.datetime]
...
dfks['datetime_pdt']=[dt.astimezone(utc) for dt in dfks.datetime]
...
with Image.open(figname) as img:
        ...
        if (dt>ot0)&(dt<ot1):
            ...
            if np.sum(zmask)>0:
                ...
                if np.any(dfs.approx_dt==dtts):
                    t2=argnearest(dfs.approx_dt,dtts)
                    reflon = dfs.long[t2]
                    reflat = dfs.lat[t2]
                    ...
                    if allmaskcount>0:
                        nearby_count+=allmaskcount
                        ax.scatter(ds['lon'][t,allmask],ds['lat'][t,allmask],c='orange',s=1,alpha=1.0,zorder=105)
                
        ds.close()
    ax.axis(img_extent)
        
    ax.text(0.9,0.1,dt.astimezone(Pacific).strftime("%m/%d/%Y, %H:%M:%S PDT")+'\nArrow = current, circle = 50 cm/s\nNOAA PORTS station ks0201 Bangor, Hood Canal',transform=ax.transAxes,ha='right',va='center',color='white')

    # add tide plot
    axt = plt.subplot(gs[0,1])
    axt.plot(dft.datetime_pdt,dft.Pred-np.mean(dft.Pred),color=tab10(0))
    t = argnearest(dft.datetime,dtts)
    axt.plot(dft.datetime_pdt.values[t],dft.Pred[t]-np.mean(dft.Pred),marker='o',mec='k',mfc=tab10(0),markersize=10)
    ...
    ylim=axt.get_ylim()
    # xlim=axt.get_xlim()
    axt.text(0.1,0.9,'a) Tidal height\nNOAA tide prediction for 9445133, Bangor WA',transform=axt.transAxes)
    ...
    axs.plot(dfs.mid_dt_pdt[dfs.approx_dt>dt],dfs.eDNA_present[dfs.approx_dt>dt],linestyle='None',marker='o',mec='gray',mfc='None',markersize=10)
    if dt>=first_sample_utc:
        ...
        axs.scatter(dfs.mid_dt_pdt[dfs.approx_dt<dt],dfs.eDNA_present[dfs.approx_dt<dt],marker='o',s=100,edgecolors='k',c=dfs.eDNA_present[dfs.approx_dt<dt],cmap=yesno_cmap,zorder=200)
        if np.any(dfs.approx_dt==dtts):
            ...
            axs.plot(dfs.mid_dt_pdt[t],dfs.eDNA_present[t],linestyle='None',marker='o',mfc=yesno_col_list[int(dfs.eDNA_present[t])],mec='k',markersize=12)
            print(f'{nearby_count} particles within {refdepth} m of surface and {refradius} m radius of sampling station {dfs.sampling_location[t]}')
    # axs.set_xlim(xlim)
    axs.set_xlabel('Time (PDT)')
    axs.set_ylabel('')
    axs.set_yticks([0,1])
    ...
    for ax in axt, axs:
        ...
        ax.set_xlim([first_sample_utc.astimezone(Pacific),last_sample_utc.astimezone(Pacific)])


    outfn = home+'etools/plots/June5_sampling_location_onetimestep.png'
    plt.savefig(outfn)
