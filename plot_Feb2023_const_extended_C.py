...
for ind in ns_inds:
    ...
    if moor['label'] in mll2ref.keys():
        ...
        ax_samp.plot(moor['dist'],samples[moor_count],marker='d',mfc=rainbow(moor_count),mec='k',linestyle='None',markersize=14,alpha=1.0,zorder=15)
        ax_samp.plot(moor['dist']*np.ones((nsamples)),sampledata,marker='d',mfc=rainbow(moor_count),mec='None',linestyle='None',markersize=10,alpha=0.5,zorder=5)
        
        # ax_samp2.plot(dist,particles[moor_count],marker='*',mfc=rainbow(moor_count),mec='k',linestyle='None',markersize=10,alpha=1.0,zorder=15)
...
for moor in moor_list:
    if moor['label'] in mll2ref.keys():
        ax_samp.plot(moor['dist'],slope*particles[moor_count],marker='*',mfc=rainbow(moor_count),mec='k',linestyle='None',markersize=14,alpha=1.0,zorder=15)
        moor_count+=1
...
p=ax.pcolormesh(xbins,ybins,slope*moor_dict['const_DNA_bin'][sample_dt_ind,:].T,cmap=cmo.cm.matter,norm=matplotlib.colors.LogNorm(),zorder=1,shading='nearest')
