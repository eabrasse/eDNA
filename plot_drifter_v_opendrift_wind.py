...
relnum = 3
lag = 0
opendrift_fn_list = [home+'data/opendrift/wind_tests/HCdrifter_LOcurrents_nowinds.nc',home+'data/opendrift/wind_tests/HCdrifter_LOcurrents_LOwinds.nc',home+'data/opendrift/wind_tests/HCdrifter_LOcurrents_NOAAwinds.nc']
label_list = ['No wind','LO wind', 'NOAA wind']
fs = 12
...
drifty1 = meta_df['Recovery y'].loc[rel]
 # distance of particle end points from drifter end points

with Image.open(figname) as img:

    fw,fh = efun.gen_plot_props()
    fig = plt.figure(figsize=(fw*5,fh*1.7))
    gs = GridSpec(2,len(opendrift_fn_list))
    
    # plot wind difference
    axwind = plt.subplot(gs[1,:])
    
    
    count=0
    for opendrift_fn in opendrift_fn_list:
        ...
        if count==0:
            ...
            pt_list = []
            for ptt in pt:
                pt_list.append(utc.localize(datetime(1970,1,1)+timedelta(seconds=ptt)).astimezone(pdt))
        # px1, py1 = efun.ll2xy(ds['lon'][:,-1],ds['lat'][:,-1],lon0,lat0)
        # pr1 = np.sqrt((px1-driftx1)**2+(py1-drifty1)**2)
        
        axwind.quiver(pt_list,np.zeros((ntime)),np.mean(ds['x_wind'],axis=0),np.mean(ds['y_wind'],axis=0),scale_units='y',
              scale=1.0,width=0.002, headwidth=2, color=tab10(count),zorder=qzo)
        axwind.text(0.1,0.9-0.1*count,label_list[count],transform=axwind.transAxes,ha='left',va='top',color=tab10(count))
        
        ax = plt.subplot(gs[0,count],projection=ccrs.PlateCarree())
        ...
        ax.text(0.9,0.9,label_list[count],transform=ax.transAxes,ha='right',va='top',color=tab10(count),bbox=dict(fc="white", ec="None",lw=2))
        if count==0:
        ...
        ds.close()
        count+=1
    axwind.set_xlabel('2024 Date, time (PDT)',fontsize=fs)
    axwind.xaxis.set_major_formatter(mdates.DateFormatter("%b %-d, %H:%M",tz=pdt))
    axwind.set_ylim([-4,4])
    axwind.axhline(y=0,color='gray',zorder=1,lw=0.5)
    axwind.set_ylabel('Wind speed (m/s)',fontsize=fs)
    axwind.tick_params(axis='both', which='major', labelsize=fs)
