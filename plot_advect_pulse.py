...
for ax,k in [[axs[0,0],k_constant],[axs[1,0],k_constant_v],[axs[0,1],k_pulse],[axs[1,1],k_pulse_v]]:
...
for pax in [pax2,pax3]:
    ...
    pax.set_ylim([-15,60])
for pax in [pax0,pax1,pax2,pax3]:
...
for vax in [vax0,vax2]:
    vax.plot(t,v0,color=tab20c(4))
for vax in [vax1,vax3]:
    vax.plot(t,v,color=tab20c(4))
for vax in [vax0,vax1,vax2,vax3]:
    vax.set_ylabel('Current',fontsize=6)
    vax.set_ylim(-3,3)
    vax.set_xlabel('Time',fontsize=6)

for pvax in [pax0,pax1,pax2,pax3,vax0,vax1,vax2,vax3]:
    pvax.set_xticks([])
    pvax.set_yticks([0])
