...
# ax2.plot(xscale*(0.5*(ESP['df']['timestamp_0']+ESP['df']['timestamp_1'])-ts0),np.log(np.array(ESP['df']['corr_conc'])),linestyle='-',marker='o',mfc='k',mec='None',color='k',lw=0.5,markersize=3)
...
ax2.fill_between(xscale*(dolphin['df']['timestamp']-ts0),dolphin['df']['ndolphin_net'],color=dolphincolor,edgecolor='none',alpha=.3,zorder=1,)
hap_div = np.zeros(len(mifish['df'].index))
for i in mifish['df'].index:
    ...
    for column in column_sub:
        ...
        ax.fill_between([xscale*(mifish['df']['timestamp_0'][i]-ts0),xscale*(mifish['df']['timestamp_1'][i]-ts0)],y1=[running_sum]*2,y2=[running_sum+newcount]*2,
            alpha=alp,color=mifish_cmap(hap_count),zorder=250,edgecolor='none',hatch=hatchstyle)#,lw=4)
...
ax2.set_ylabel('N (dolphins)')
# ax2.set_yticks([0,1,2,3])
ax.set_ylabel('log MiFish # Tursiops Truncatus reads')
...
ax3 = fig2.gca()
ax3.fill_between(xscale*(dolphin['df']['timestamp']-ts0),dolphin['df']['ndolphin_net'],color=dolphincolor,edgecolor='none',alpha=.3,zorder=1,)
ax3.fill_between(xscale*(0.5*(mifish['df']['timestamp_0']+mifish['df']['timestamp_1'])-ts0),hap_div,color='none',edgecolor=dolphincolor,alpha=1,zorder=100,hatch='//')
ax3.set_xlim([xscale*(mifish['df']['timestamp_0'].min()-fudge-ts0),xscale*(mifish['df']['timestamp_1'].max()+fudge-ts0)])
ax3.set_ylabel('Count')
ax3.set_yticks([0,1,2,3])
ax3.set_xlabel('Hours since start of ESP sampling')
