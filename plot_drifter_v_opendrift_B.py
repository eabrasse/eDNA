...
a_ll = (-122.738, -122.728, 47.736877, 47.744494)
...
with Image.open(figname) as img:
    ...
    fig = plt.figure(figsize=(fw*6,fh*1.))
    gs = GridSpec(1,len(rel_list))
    lag = 0

    for relnum in rel_list:
        
        opendrift_fn = home+'data/opendrift/wind_tests/HCdrifter_rel{}_1hrlag_corrected_nowind.nc'.format(relnum)
        plottitle='Delta Pier masked'
        ...
        ax = plt.subplot(gs[relnum-1],projection=ccrs.PlateCarree())
        ax.imshow(img, extent=img_extent,transform=ccrs.PlateCarree())
        # ax.imshow(img,extent=a_ll,transform=ccrs.PlateCarree())
        ...
        for i in range(nseed):
            ...
            lc = LineCollection(segments,linestyle='solid',color='w',alpha=0.2,linewidth=0.1,zorder=100)
        ...
        for ind in rinds:
            ...
            line = ax.add_collection(lc)
            ax.plot(ds['lon'][ind,0],ds['lat'][ind,0],marker='o',lw=3,markersize=5,mec=Set2(int(rel)),mfc='w',zorder=200)
        ...
        if rel>0:
            ax.plot(meta_df['Release Lon'].loc[rel],meta_df['Release Lat'].loc[rel],linestyle='none',marker='v',mec='k',mfc=Dark2(int(rel)),markersize=10,zorder=350)
        ax.plot(lons,lats,linestyle='solid',marker='none',color=Dark2(int(rel)),lw=1.5,zorder=350)
        ax.plot(ddf['  Longitude'],ddf['   Latitude'],linestyle='none',marker='o',mec=Dark2(int(rel)),color=Dark2(int(rel)),mfc=Dark2(int(rel)),markersize=5,zorder=330)
        ax.plot(meta_df['Recovery Lon'].loc[rel],meta_df['Recovery Lat'].loc[rel],linestyle='none',marker='^',mec='k',mfc=Dark2(int(rel)),markersize=10,zorder=360)
        ax.set_xlabel('Longitude')
        ax.set_ylabel('Latitude')

        # ax.axis(a_ll)
        ax.set_extent(a_ll,crs=ccrs.PlateCarree())
    
        textstr0 = 'Drifter Release #{}\nStation: {}\nTime in: {}\nTime out: {}'.format(rel,meta_df['Station'].loc[rel],dt0.strftime("%H:%M PDT"),dt1.strftime("%H:%M PDT"))
        ax.text(0.05,0.95,textstr0,transform=ax.transAxes,ha='left',va='top',color=Dark2(int(rel)),size=fs,fontweight='bold')
        # textstr1 = 'Particles\n\nTime in: {}\nTime out: {}'.format(pt0.strftime("%H:%M PDT"),pt1.strftime("%H:%M PDT"))
        # ax.text(0.3,0.95,textstr1,transform=ax.transAxes,ha='left',va='top',color='w',size=fs)
        

        if relnum==1:
            ax.text(0.1,1.05,plottitle,transform=ax.transAxes,ha='left',fontsize=12,zorder=600)
